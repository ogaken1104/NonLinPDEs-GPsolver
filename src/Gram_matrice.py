...
from .kernels import Anisotropic_Gaussian_kernel, Gaussian_kernel
...
def Gram_matrix_assembly(
    ...
    if eqn == "Nonlinear_elliptic":
        ...
        val = vmap(
            lambda x1, x2, y1, y2: K.Delta_x_Delta_y_kappa(
                x1, x2, y1, y2, kernel_parameter
            )
        )(
            XXdd0.flatten(),
            XXdd1.flatten(),
            onp.transpose(XXdd0).flatten(),
            onp.transpose(XXdd1).flatten(),
        )
        ...
        val = vmap(lambda x1, x2, y1, y2: K.kappa(x1, x2, y1, y2, kernel_parameter))(
            XXdbdb0.flatten(),
            XXdbdb1.flatten(),
            onp.transpose(XXdbdb0).flatten(),
            onp.transpose(XXdbdb1).flatten(),
        )
        Theta[N_domain:, N_domain:] = onp.reshape(
            val, (N_domain + N_boundary, N_domain + N_boundary)
        )

        # interior v.s. interior+boundary
        val = vmap(
            lambda x1, x2, y1, y2: K.Delta_x_kappa(x1, x2, y1, y2, kernel_parameter)
        )(XXddb0.flatten(), XXddb1.flatten(), XXddb0_2.flatten(), XXddb1_2.flatten())
        Theta[:N_domain, N_domain:] = onp.reshape(
            val, (N_domain, N_domain + N_boundary)
        )
        Theta[N_domain:, :N_domain] = onp.transpose(
            onp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        return Theta
        ...
        val = vmap(
            lambda x1, x2, y1, y2: K.D_x1_DD_y2_kappa(x1, x2, y1, y2, kernel_parameter)
        )(
            XXdd0.flatten(),
            XXdd1.flatten(),
            jnp.transpose(XXdd0).flatten(),
        ...
        Theta = Theta.at[0:N_domain, 2 * N_domain : 3 * N_domain].set(
            jnp.reshape(val, (N_domain, N_domain))
        )
        Theta = Theta.at[2 * N_domain : 3 * N_domain, 0:N_domain].set(
            jnp.transpose(jnp.reshape(val, (N_domain, N_domain)))
        )
        ...
        val = vmap(
            lambda x1, x2, y1, y2: K.D_x2_DD_y2_kappa(x1, x2, y1, y2, kernel_parameter)
        )(
            XXdd0.flatten(),
            XXdd1.flatten(),
            jnp.transpose(XXdd0).flatten(),
        ...
        Theta = Theta.at[N_domain : 2 * N_domain, 2 * N_domain : 3 * N_domain].set(
            jnp.reshape(val, (N_domain, N_domain))
        )
        Theta = Theta.at[2 * N_domain : 3 * N_domain, N_domain : 2 * N_domain].set(
            jnp.transpose(jnp.reshape(val, (N_domain, N_domain)))
        )

        val = vmap(
            lambda x1, x2, y1, y2: K.DD_x2_DD_y2_kappa(x1, x2, y1, y2, kernel_parameter)
        )(
            XXdd0.flatten(),
            XXdd1.flatten(),
            jnp.transpose(XXdd0).flatten(),
        ...
        Theta = Theta.at[2 * N_domain : 3 * N_domain, 2 * N_domain : 3 * N_domain].set(
            jnp.reshape(val, (N_domain, N_domain))
        )
        ...
        Theta = Theta.at[3 * N_domain :, 3 * N_domain :].set(
        ...
        Theta = Theta.at[0:N_domain, 3 * N_domain :].set(
            jnp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        Theta = Theta.at[3 * N_domain :, 0:N_domain].set(
        ...
        Theta = Theta.at[N_domain : 2 * N_domain, 3 * N_domain :].set(
            jnp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        Theta = Theta.at[3 * N_domain :, N_domain : 2 * N_domain].set(
            jnp.transpose(onp.reshape(val, (N_domain, N_domain + N_boundary)))
        )

        val = vmap(
            lambda x1, x2, y1, y2: K.DD_x2_kappa(x1, x2, y1, y2, kernel_parameter)
        )(XXddb0.flatten(), XXddb1.flatten(), XXddb0_2.flatten(), XXddb1_2.flatten())
        Theta = Theta.at[2 * N_domain : 3 * N_domain, 3 * N_domain :].set(
            jnp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        Theta = Theta.at[3 * N_domain :, 2 * N_domain : 3 * N_domain].set(
            jnp.transpose(jnp.reshape(val, (N_domain, N_domain + N_boundary)))
        )
        return Theta
    elif eqn == "Eikonal":
        ...
        Theta[N_domain : 2 * N_domain, N_domain : 2 * N_domain] = jnp.reshape(
            val, (N_domain, N_domain)
        )
        val = vmap(
            lambda x1, x2, y1, y2: K.D_x1_D_y2_kappa(x1, x2, y1, y2, kernel_parameter)
        )(
        ...
        Theta[:N_domain, N_domain : 2 * N_domain] = jnp.reshape(
            val, (N_domain, N_domain)
        )
        Theta[N_domain : 2 * N_domain, :N_domain] = jnp.transpose(
            jnp.reshape(val, (N_domain, N_domain))
        )
        ...
        Theta[2 * N_domain : 3 * N_domain, 2 * N_domain : 3 * N_domain] = jnp.reshape(
            val, (N_domain, N_domain)
        )
        val = vmap(
            lambda x1, x2, y1, y2: K.Delta_x_D_y1_kappa(
                x1, x2, y1, y2, kernel_parameter
            )
        )(
            XXdd0.flatten(),
        ...
        Theta[2 * N_domain : 3 * N_domain, :N_domain] = jnp.reshape(
            val, (N_domain, N_domain)
        )
        Theta[:N_domain, 2 * N_domain : 3 * N_domain] = jnp.transpose(
            jnp.reshape(val, (N_domain, N_domain))
        )
        val = vmap(
            lambda x1, x2, y1, y2: K.Delta_x_D_y2_kappa(
                x1, x2, y1, y2, kernel_parameter
            )
        )(
            XXdd0.flatten(),
        ...
        Theta[2 * N_domain : 3 * N_domain, N_domain : 2 * N_domain] = jnp.reshape(
            val, (N_domain, N_domain)
        )
        Theta[N_domain : 2 * N_domain, 2 * N_domain : 3 * N_domain] = jnp.transpose(
            jnp.reshape(val, (N_domain, N_domain))
        )
        ...
        Theta[3 * N_domain :, 3 * N_domain :] = jnp.reshape(
            val, (N_domain + N_boundary, N_domain + N_boundary)
        )
        ...
        Theta[:N_domain, 3 * N_domain :] = jnp.reshape(
            val, (N_domain, N_domain + N_boundary)
        )
        Theta[3 * N_domain :, :N_domain] = jnp.transpose(
            jnp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        ...
        Theta[N_domain : 2 * N_domain, 3 * N_domain :] = jnp.reshape(
            val, (N_domain, N_domain + N_boundary)
        )
        Theta[3 * N_domain :, N_domain : 2 * N_domain] = jnp.transpose(
            jnp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        ...
        Theta[2 * N_domain : 3 * N_domain, 3 * N_domain :] = jnp.reshape(
            val, (N_domain, N_domain + N_boundary)
        )
        Theta[3 * N_domain :, 2 * N_domain : 3 * N_domain] = jnp.transpose(
            jnp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        return Theta
        ...
        Theta_u[N_domain : 2 * N_domain, N_domain : 2 * N_domain] = onp.reshape(
            val, (N_domain, N_domain)
        )

        val = vmap(
            lambda x1, x2, y1, y2: K.D_x2_D_y1_kappa(x1, x2, y1, y2, kernel_parameter)
        )(
            XXdd0.flatten(),
            XXdd1.flatten(),
            onp.transpose(XXdd0).flatten(),
        ...
        Theta_u[N_domain : 2 * N_domain, 0:N_domain] = onp.reshape(
            val, (N_domain, N_domain)
        )
        Theta_u[0:N_domain, N_domain : 2 * N_domain] = onp.transpose(
            onp.reshape(val, (N_domain, N_domain))
        )
        ...
        Theta_u[2 * N_domain : 3 * N_domain, 2 * N_domain : 3 * N_domain] = onp.reshape(
            val, (N_domain, N_domain)
        )
        ...
        Theta_u[2 * N_domain : 3 * N_domain, 0:N_domain] = onp.reshape(
            val, (N_domain, N_domain)
        )
        Theta_u[0:N_domain, 2 * N_domain : 3 * N_domain] = onp.transpose(
            onp.reshape(val, (N_domain, N_domain))
        )
        ...
        Theta_u[2 * N_domain : 3 * N_domain, N_domain : 2 * N_domain] = onp.reshape(
            val, (N_domain, N_domain)
        )
        Theta_u[N_domain : 2 * N_domain, 2 * N_domain : 3 * N_domain] = onp.transpose(
            onp.reshape(val, (N_domain, N_domain))
        )
        ...
        Theta_u[3 * N_domain :, 3 * N_domain :] = onp.reshape(
            val, (N_domain + N_boundary, N_domain + N_boundary)
        )
        ...
        Theta_u[0:N_domain, 3 * N_domain :] = onp.reshape(
            val, (N_domain, N_domain + N_boundary)
        )
        Theta_u[3 * N_domain :, 0:N_domain] = onp.transpose(
            onp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        ...
        Theta_u[N_domain : 2 * N_domain, 3 * N_domain :] = onp.reshape(
            val, (N_domain, N_domain + N_boundary)
        )
        Theta_u[3 * N_domain :, N_domain : 2 * N_domain] = onp.transpose(
            onp.reshape(val, (N_domain, N_domain + N_boundary))
        )
        ...
        Theta_u[2 * N_domain : 3 * N_domain, 3 * N_domain :] = onp.reshape(
            val, (N_domain, N_domain + N_boundary)
        )
        Theta_u[3 * N_domain :, 2 * N_domain : 3 * N_domain] = onp.transpose(
            onp.reshape(val, (N_domain, N_domain + N_boundary))
        )

        # Construct kernel matrix Theta_a
        # interior v.s. interior
        Theta_a[0 : 2 * N_domain, 0 : 2 * N_domain] = Theta_u[
            0 : 2 * N_domain, 0 : 2 * N_domain
        ]
        Theta_a[2 * N_domain : 3 * N_domain, 2 * N_domain : 3 * N_domain] = Theta_u[
            3 * N_domain : 4 * N_domain, 3 * N_domain : 4 * N_domain
        ]
        Theta_a[0 : 2 * N_domain, 2 * N_domain : 3 * N_domain] = Theta_u[
            0 : 2 * N_domain, 3 * N_domain : 4 * N_domain
        ]
        Theta_a[2 * N_domain : 3 * N_domain, 0 : 2 * N_domain] = Theta_u[
            3 * N_domain : 4 * N_domain, 0 : 2 * N_domain
        ]
...
def construct_Theta_test(
    ...
    if eqn == "Nonlinear_elliptic":
        ...
        val = vmap(
            lambda x1, x2, y1, y2: K.DD_y2_kappa(x1, x2, y1, y2, kernel_parameter)
        )(XXtd0.flatten(), XXtd1.flatten(), XXtd0_2.flatten(), XXtd1_2.flatten())
        Theta_test[:, 2 * N_domain : 3 * N_domain] = onp.reshape(
            val, (N_test, N_domain)
        )
        ...
        Theta_test[:, 3 * N_domain : 4 * N_domain + N_boundary] = onp.reshape(
        ...
        Theta_test[:, 3 * N_domain :] = onp.reshape(
            val, (N_test, N_domain + N_boundary)
        )
        return Theta_test
    elif eqn == "Darcy_flow2d":
        ...
        Theta_u_test[:, 2 * N_domain : 3 * N_domain] = onp.reshape(
            val, (N_test, N_domain)
        )
        ...
        Theta_u_test[:, 3 * N_domain :] = onp.reshape(
            val, (N_test, N_domain + N_boundary)
        )
