...
from src_new.solver_new import solver_GP
...
def get_parser():
    ...
    parser.add_argument(
        "--kernel_parameter", type=float, nargs="+", default=[0.3, 0.05]
    )
...
alpha = cfg.alpha
nu = cfg.nu
solver = solver_GP(cfg, PDE_type="Burgers", label="few_z")
...
def u_truth(x1, x2):
    ...
    val1 = (
        weights
        * jnp.sin(jnp.pi * temp)
        * jnp.exp(-jnp.cos(jnp.pi * temp) / (2 * jnp.pi * nu))
    )
...
if cfg.show_figure:
    ...
    solver.contour_of_three_plot(XX, YY)
