...
def get_parser():
    ...
    # sampling points
    parser.add_argument(
        "--sampled_type", type=str, default="random", choices=["random", "grid"]
    )
...
solver.auto_sample_IP(
    cfg.N_domain, cfg.N_boundary, cfg.N_data, sampled_type=cfg.sampled_type
)
if cfg.show_figure:
...
def a(x1, x2):  # a(x) truth
    ...
    ) + jnp.exp(-c * jnp.sin(2 * jnp.pi * x1) - c * jnp.sin(2 * jnp.pi * x2))
...
test_truth_a = onp.reshape(
    vmap(a)(X_test[:, 0], X_test[:, 1]), (N_pts_per_dim, N_pts_per_dim)
)
...
if cfg.show_figure:
    ...
    plt.tight_layout()
    plt.savefig("fig/Darcy_flow2d_result.png", dpi=300, bbox_inches="tight")
